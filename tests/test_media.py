...
def test_upload(session, client, mocker, login):
    ...
    expected_original_filename = secure_filename(
        str(shared_data.ASSET_DIR/"anders-jilden.jpg"))

    assert resp.json["filename"] == expected_original_filename
    assert resp.json["size"] == os.stat((shared_data.ASSET_DIR / "anders-jilden.jpg")).st_size
    assert resp.json["mime_type"] == "image/jpeg"

    # query db to ensure files exist
    assert session.query(models.Media).count() == 2
    assert session.query(models.Images).count() == 2
    # ensure no posts have been created
    assert session.query(models.PostTable).count() == 0

    # grab media and ensure it was actually saved
    orig_file = session.query(models.Media).filter_by(type="image").first()
    
    assert orig_file is not None
    assert orig_file.orphaned == True
    assert orig_file.image_uuid is not None
    assert orig_file.video_uuid is None
    assert orig_file.type == type_
    # ensure we've saved sanitized version of the original name
    assert orig_file.original_filename == expected_original_filename
    # ensure thumbnail was saved
    assert orig_file.thumbnail_uuid is not None

    # ensure image was saved in original size
    image = session.query(models.Images).filter_by(uuid=orig_file.image_uuid).first()
    ...
    assert len([p for p in expected_file_path.iterdir()]) == 1
    for file in expected_file_path.iterdir():
        # assert we've saved some data successfully
        assert os.stat(file).st_size >= 1024
        # make sure the name we save is not the original filename, should be
        # autogenerated
        assert file.name != expected_original_filename

    # --------- ensure thumbnail ---------
    thumb_file = session.query(models.Media).filter_by(type="thumbnail").first()
    
    assert thumb_file is not None
    assert thumb_file.orphaned == False
    assert thumb_file.image_uuid is not None
    assert thumb_file.video_uuid is None
    assert thumb_file.type == "thumbnail"
    # ensure thumbnail was saved
    assert thumb_file.thumbnail_uuid is None

    # ensure image was saved in original size
    image = session.query(models.Images).filter_by(uuid=thumb_file.image_uuid).first()
...
def test_upload_video(session, client, mocker, login):
    ...
    expected_original_filename = secure_filename(
        str(shared_data.ASSET_DIR/"big-bunny.mp4"))

    assert resp.json["filename"] == expected_original_filename
    assert resp.json["size"] == os.stat((shared_data.ASSET_DIR / "big-bunny.mp4")).st_size
    ...
    orig_file = session.query(models.Media).filter_by(type="video").first()
    
    assert orig_file is not None
    assert orig_file.orphaned == True
    assert orig_file.image_uuid is None
    assert orig_file.video_uuid is not None
    assert orig_file.type == type_
    # ensure we've saved sanitized version of the original name
    assert orig_file.original_filename == expected_original_filename
    # ensure thumbnail was saved
    ...
    # ensure video was saved in original size
    video = session.query(models.Videos).filter_by(uuid=orig_file.video_uuid).first()
...
def test_upload_gif_with_dot_gif_extension(session, client, mocker, login):
    ...
    expected_original_filename = secure_filename(
        str(shared_data.ASSET_DIR/"earth.gif"))

    assert resp.json["filename"] == expected_original_filename
    assert resp.json["size"] == os.stat((shared_data.ASSET_DIR / "earth.gif")).st_size
    assert resp.json["mime_type"] == "image/gif"
    ...
    assert image is not None
    # this file is actually smaller than our expected min so thumb should stay
    # the same size
    assert image.height == 400
    assert image.width == 400
...
def test_upload_image_with_webp_format(client, mocker, login):
    ...
    assert resp.json["filename"] == secure_filename(str(filename))
    assert resp.json["size"] == os.stat(filename).st_size
...
def test_random_avatar_gen(client, session, login):
    ...
    assert session.query(models.Media).filter_by(type="avatar").count() == 6
    # this should be 2 * 7 as there are thumbnails for each
    assert session.query(models.Images).count() == 14
...
def test_chunk_upload__merge(mocker, client, login):
    ...
    assert resp.json["filename"] == "big-bunny.mp4"
    assert resp.json["size"] == file_size
    assert resp.json["mime_type"] == "video/mp4"
...
def test_chunk_upload__merge_ensure_cleanup(mocker, client, session, login):
    ...
    # make sure db is ok
    assert session.query(models.Media).count() == 2
    # ensure thumbnail was created
    assert session.query(models.Images).count() == 1
    # ensure no posts have been created
    assert session.query(models.PostTable).count() == 0

    media = session.query(models.Media).filter_by(type="video").first()
    assert media is not None
    assert media.type == "video"
    assert media.image_uuid is None
    assert media.thumbnail_uuid is not None
